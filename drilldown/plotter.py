from vtk import vtkDataSetMapper, vtkExtractSelection, vtkSelectionNode, vtkSelection, vtkPropPicker, vtkCellPicker, vtkCellLocator, vtkHardwareSelector, vtkDataObject, vtkCoordinate
...
class DrillDownPlotter(Plotter): 
    ...
    def _make_selectable(self, actor, color_on_selection="#000000", opacity_on_selection=0.95, accelerated_selection=False):
        # make pickable
        actor.SetPickable = 0
            ...
            if picked_actor == self._actors["drillhole intervals"]: 
                ...
                if picked_interval_cell is not None:
                    ...
                    if shift_pressed == True: 
                        self._make_multi_selection(picked_interval_cell, continuous=True)
                    elif ctrl_pressed == True: 
                        self._make_multi_selection(picked_interval_cell, continuous=False)
                    else: 
    ...
    def _make_single_selection(self, picked_interval_cell): 
            selected_interval = int(np.floor(picked_interval_cell / self.faces_per_interval))
            selected_interval_cells = np.arange(selected_interval * self.faces_per_interval, (selected_interval + 1) * self.faces_per_interval).tolist()
    ...
    def _make_multi_selection(self, picked_interval_cell, continuous=False): 
        if continuous==True: 
            selected_intervals, selected_interval_cells = self._make_continuous_multi_selection(picked_interval_cell)
        else: 
            selected_intervals, selected_interval_cells = self._make_discontinuous_multi_selection(picked_interval_cell)
    ...
    def _make_continuous_multi_selection(self, picked_interval_cell): 
        if self._picked_interval_cell < picked_interval_cell: # normal direction (down the hole)
            selected_intervals = np.arange(self._selected_intervals[-1] + 1, int(np.floor(picked_interval_cell / self.faces_per_interval)) + 1).tolist()
            selected_interval_cells = np.arange((selected_intervals[0]) * self.faces_per_interval, (selected_intervals[-1] + 1) * self.faces_per_interval).tolist()
        
        else: # reverse direction (up the hole)
            selected_intervals = np.arange(int(np.floor(picked_interval_cell / self.faces_per_interval)), self._selected_intervals[-1]).tolist()
            selected_interval_cells = np.arange((selected_intervals[0] * self.faces_per_interval), (selected_intervals[-1] + 1) * self.faces_per_interval).tolist()
    ...
    def _update_selection_object(self, interval_or_sample, selected_cells): 
            ...
            sel_mapper.SetRelativeCoincidentTopologyPolygonOffsetParameters(0., -10)
            ...
            self.selection_actor, _ = self.add_actor(sel_actor, name="drillhole intervals selection", reset_camera=False, pickable=False)
            self.render()
    ...
    def update_active_var(self, active_var): 
        self._actors["drillhole intervals"].mapper.dataset.set_active_scalars(active_var)    
        self.render()
    ...
    def update_cmap_range(self, cmap_range): 
        self._actors["drillhole intervals"].mapper.lookup_table.scalar_range = cmap_range
